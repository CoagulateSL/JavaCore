...
public class HTTPSListener {
	...
	public HTTPSListener(final int port,@Nonnull final String pemfile,final HttpRequestHandlerMapper mapper) {
		...
		try {
			...
			final byte[] certBytes=
					CertUtils.parseDERFromPEM(certAndKey,"-----BEGIN CERTIFICATE-----","-----END CERTIFICATE-----");
			final byte[] keyBytes=
					CertUtils.parseDERFromPEM(certAndKey,"-----BEGIN PRIVATE KEY-----","-----END PRIVATE KEY-----");
			final byte[] chainBytes=
					CertUtils.parseDERFromPEM(Files.readAllBytes(new File("/etc/keys/chain.pem").toPath()),
					                          "-----BEGIN CERTIFICATE-----",
					                          "-----END CERTIFICATE-----");
			...
			if (server==null) {
				throw new SystemInitialisationException("Server bootstrap was null?");
			}
	...
	private static class ShutdownHook extends Thread {
		...
		public ShutdownHook(final HTTPSListener target) {
			this.target=target;
		}
