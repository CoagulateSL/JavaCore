...
public abstract class ExceptionTools {
	...
	public static String dumpException(@Nonnull final Throwable e) {
		...
		for (final StackTraceElement st: e.getStackTrace()) {
			if (!st.getClassName().startsWith("net.coagulate.")) {
				p.append("<del>");
			}
			p.append(" at ")
	...
	public static String toString(@Nonnull final Throwable e) {
		...
		while (loop!=null) {
			p.append("Exception: ")
			 .append(loop.getClass().getSimpleName())
			 .append(" - ")
			 .append(loop.getLocalizedMessage())
			 .append("\n");
	...
	@Nonnull
	public static String toHTML(@Nonnull final Throwable e) {
	...
	public static String getPertinent(final Throwable t) {
		...
		return prefix+"["+frame.getClassName().replaceFirst("net.coagulate.","")+"."+frame.getMethodName()+":"+
		       frame.getLineNumber()+"] - "+t.getClass().getSimpleName()+" - "+t.getLocalizedMessage();
	}
}
